...
function loadnavigationMenu()
{
  ...
  navigationMenu.innerHTML = '<a href="../index.php">Home</a> > ' + '<a href="../categories/category.php?category=' + category + '&catid=' + catid  + '">' + category + '</a>' +'> ' + name;
...
function loadProductDetail()
{
    ...
    let mainContent = new XMLHttpRequest();
    mainContent.open("POST","../admin_process.php?action=fetch_detail_for_one_product",true);
    mainContent.setRequestHeader("Content-type","application/x-www-form-urlencoded");

    let htmlPart = '';
    mainContent.onload = function()
    {
        let response = this.responseText.replace('while(1);','');
        let result = JSON.parse(response);
        htmlPart = '<div class="col">' +
        '<img src="../../images/' + category + '/' + result['success'][0]['name'].replaceAll(/[^a-zA-Z\d]/g, "").replaceAll(" ","_") + '.jpg" class="img-fluid" alt="image">' +
      '</div>' + 
      '<div class="col">' +
        '<p>' + result['success'][0]['name'] +'</p>' + 
        '<p class="price"> HK$' + result['success'][0]['price'] + '</p>' + 
        '<p class="showStock"><b>In Stock</b></p>' +
       '<div class="row">' +
          '<div class="col">' +
           '<form>' +
              '<div class="form-group">' +
                '<label for="productNumber">Numbers</label>'+
               '<input type="number" class="form-control" id="productNumber" placeholder="Enter products Numbers" value="1" min="1">'+
             '</div>'+
            '</form>' +
          '</div>' +
          '<div class="col addToCartButton" >' +
        '<button type="button" class="btn btn-primary" onclick="addAtProductPage(' + result['success'][0]['pid'] + ')">' +
      ...
      '<div>' +
        '<p>Product details:</p>' +
          '<ul class="list-group list-group-flush">' +
            '<li class="list-group-item">' + result['success'][0]['description'] + '</li>' +
          '</ul>' +
          '<br/>' +
